...
public:
	 ...
	 */
	FORCEINLINE TMovieSceneChannelData<FEvtCharaAnimationPayload> GetData()
	{
		return TMovieSceneChannelData<FEvtCharaAnimationPayload>(&Times, &KeyValues, &KeyHandles);
	}
	 ...
	 */
	FORCEINLINE TMovieSceneChannelData<const FEvtCharaAnimationPayload> GetData() const
	{
		return TMovieSceneChannelData<const FEvtCharaAnimationPayload>(&Times, &KeyValues);
	}
	...
	bool Evaluate(FFrameTime InTime, FEvtCharaAnimationPayload& OutValue) const { return true; }
...
public:
	...
	const TArray<FEvtCharaAnimationPayload>& GetValues() const { return KeyValues; }
public:
    XRD777_API FMovieSceneEvtCharaAnimationSectionData();
};

template<>
struct TMovieSceneChannelTraits<FMovieSceneEvtCharaAnimationSectionData> : TMovieSceneChannelTraitsBase<FMovieSceneEvtCharaAnimationSectionData>
{
#if WITH_EDITOR
    /** Byte channels can have external values (ie, they can get their values from external objects for UI purposes) */
    typedef TMovieSceneExternalValue<FEvtCharaAnimationPayload> ExtendedEditorDataType;
#endif
};
...
{
    template<> inline FName GetKeyTypeName<FEvtCharaAnimationPayload>()
    {
        return "FEvtCharaAnimationPayload";
    }
}
#endif