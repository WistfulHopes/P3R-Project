...
public:
	 ...
	 */
	FORCEINLINE TMovieSceneChannelData<FEvtCharaPropAttachPayload> GetData()
	{
		return TMovieSceneChannelData<FEvtCharaPropAttachPayload>(&Times, &KeyValues, &KeyHandles);
	}
	 ...
	 */
	FORCEINLINE TMovieSceneChannelData<const FEvtCharaPropAttachPayload> GetData() const
	{
		return TMovieSceneChannelData<const FEvtCharaPropAttachPayload>(&Times, &KeyValues);
	}
	...
	bool Evaluate(FFrameTime InTime, FEvtCharaPropAttachPayload& OutValue) const { return true; }
...
public:
	...
	const TArray<FEvtCharaPropAttachPayload>& GetValues() const { return KeyValues; }
public:
    FMovieSceneEvtCharaPropAttachSectionData();
};

template<>
struct TMovieSceneChannelTraits<FMovieSceneEvtCharaPropAttachSectionData> : TMovieSceneChannelTraitsBase<FMovieSceneEvtCharaPropAttachSectionData>
{
#if WITH_EDITOR
    /** Byte channels can have external values (ie, they can get their values from external objects for UI purposes) */
    typedef TMovieSceneExternalValue<FEvtCharaPropAttachPayload> ExtendedEditorDataType;
#endif
};
...
{
    template<> inline FName GetKeyTypeName<FEvtCharaPropAttachPayload>()
    {
        return "FEvtCharaPropAttachPayload";
    }
}
#endif