...
public:
	 ...
	 */
	FORCEINLINE TMovieSceneChannelData<FEvtDialoguePayload> GetData()
	{
		return TMovieSceneChannelData<FEvtDialoguePayload>(&Times, &KeyValues, &KeyHandles);
	}
	 ...
	 */
	FORCEINLINE TMovieSceneChannelData<const FEvtDialoguePayload> GetData() const
	{
		return TMovieSceneChannelData<const FEvtDialoguePayload>(&Times, &KeyValues);
	}
	...
	bool Evaluate(FFrameTime InTime, FEvtDialoguePayload& OutValue) const { return true;  }
...
template<>
struct TMovieSceneChannelTraits<FMovieSceneEvtDialogueSectionData> : TMovieSceneChannelTraitsBase<FMovieSceneEvtDialogueSectionData>
{
#if WITH_EDITOR
	/** Byte channels can have external values (ie, they can get their values from external objects for UI purposes) */
	typedef TMovieSceneExternalValue<FEvtDialoguePayload> ExtendedEditorDataType;
#endif
};
...
{
	template<> inline FName GetKeyTypeName<FEvtDialoguePayload>()
	{
		return "FEvtDialoguePayload";
	}
}
#endif