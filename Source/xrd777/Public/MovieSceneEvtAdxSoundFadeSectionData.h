...
public:
	 ...
	 */
	FORCEINLINE TMovieSceneChannelData<FEvtAdxSoundFadePayload> GetData()
	{
		return TMovieSceneChannelData<FEvtAdxSoundFadePayload>(&Times, &KeyValues, &KeyHandles);
	}
	 ...
	 */
	FORCEINLINE TMovieSceneChannelData<const FEvtAdxSoundFadePayload> GetData() const
	{
		return TMovieSceneChannelData<const FEvtAdxSoundFadePayload>(&Times, &KeyValues);
	}
	...
	bool Evaluate(FFrameTime InTime, FEvtAdxSoundFadePayload& OutValue) const { return true; }
...
public:
    ...
    const TArray<FEvtAdxSoundFadePayload>& GetValues() const { return KeyValues; }
...
template<>
struct TMovieSceneChannelTraits<FMovieSceneEvtAdxSoundFadeSectionData> : TMovieSceneChannelTraitsBase<FMovieSceneEvtAdxSoundFadeSectionData>
{
#if WITH_EDITOR
	/** Byte channels can have external values (ie, they can get their values from external objects for UI purposes) */
	typedef TMovieSceneExternalValue<FEvtAdxSoundFadePayload> ExtendedEditorDataType;
#endif
};
...
{
	template<> inline FName GetKeyTypeName<FEvtAdxSoundFadePayload>()
	{
		return "FEvtAdxSoundFadePayload";
	}
}
#endif