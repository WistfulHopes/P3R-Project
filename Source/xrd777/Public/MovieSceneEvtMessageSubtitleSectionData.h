...
public:
	 ...
	 */
	FORCEINLINE TMovieSceneChannelData<FEvtMessageSubtitlePayload> GetData()
	{
		return TMovieSceneChannelData<FEvtMessageSubtitlePayload>(&Times, &KeyValues, &KeyHandles);
	}
	 ...
	 */
	FORCEINLINE TMovieSceneChannelData<const FEvtMessageSubtitlePayload> GetData() const
	{
		return TMovieSceneChannelData<const FEvtMessageSubtitlePayload>(&Times, &KeyValues);
	}
	...
	bool Evaluate(FFrameTime InTime, FEvtMessageSubtitlePayload& OutValue) const { return true; }
...
public:
	...
	const TArray<FEvtMessageSubtitlePayload>& GetValues() const { return KeyValues; }
public:
    FMovieSceneEvtMessageSubtitleSectionData();
};

template<>
struct TMovieSceneChannelTraits<FMovieSceneEvtMessageSubtitleSectionData> : TMovieSceneChannelTraitsBase<FMovieSceneEvtMessageSubtitleSectionData>
{
#if WITH_EDITOR
    /** Byte channels can have external values (ie, they can get their values from external objects for UI purposes) */
    typedef TMovieSceneExternalValue<FEvtMessageSubtitlePayload> ExtendedEditorDataType;
#endif
};
...
{
    template<> inline FName GetKeyTypeName<FEvtMessageSubtitlePayload>()
    {
        return "FEvtMessageSubtitlePayload";
    }
}
#endif