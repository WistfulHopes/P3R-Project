...
public:
	 ...
	 */
	FORCEINLINE TMovieSceneChannelData<FEvtAdxSoundPayload> GetData()
	{
		return TMovieSceneChannelData<FEvtAdxSoundPayload>(&Times, &KeyValues, &KeyHandles);
	}
	 ...
	 */
	FORCEINLINE TMovieSceneChannelData<const FEvtAdxSoundPayload> GetData() const
	{
		return TMovieSceneChannelData<const FEvtAdxSoundPayload>(&Times, &KeyValues);
	}
	...
	bool Evaluate(FFrameTime InTime, FEvtAdxSoundPayload& OutValue) const { return true; }
...
public:
	...
	const TArray<FEvtAdxSoundPayload>& GetValues() const { return KeyValues; }
public:
    FMovieSceneEvtAdxSoundSectionData();
};

template<>
struct TMovieSceneChannelTraits<FMovieSceneEvtAdxSoundSectionData> : TMovieSceneChannelTraitsBase<FMovieSceneEvtAdxSoundSectionData>
{
#if WITH_EDITOR
	/** Byte channels can have external values (ie, they can get their values from external objects for UI purposes) */
	typedef TMovieSceneExternalValue<FEvtAdxSoundPayload> ExtendedEditorDataType;
#endif
};
...
{
	template<> inline FName GetKeyTypeName<FEvtAdxSoundPayload>()
	{
		return "FEvtAdxSoundPayload";
	}
}
#endif