...
public:
	 ...
	 */
	FORCEINLINE TMovieSceneChannelData<FEvtScriptPayload> GetData()
	{
		return TMovieSceneChannelData<FEvtScriptPayload>(&Times, &KeyValues, &KeyHandles);
	}
	 ...
	 */
	FORCEINLINE TMovieSceneChannelData<const FEvtScriptPayload> GetData() const
	{
		return TMovieSceneChannelData<const FEvtScriptPayload>(&Times, &KeyValues);
	}
	...
	bool Evaluate(FFrameTime InTime, FEvtScriptPayload& OutValue) const { return true; }
...
public:
	...
	const TArray<FEvtScriptPayload>& GetValues() const { return KeyValues; }
public:
...
template<>
struct TMovieSceneChannelTraits<FMovieSceneEvtScriptSectionData> : TMovieSceneChannelTraitsBase<FMovieSceneEvtScriptSectionData>
{
#if WITH_EDITOR
    /** Byte channels can have external values (ie, they can get their values from external objects for UI purposes) */
    typedef TMovieSceneExternalValue<FEvtScriptPayload> ExtendedEditorDataType;
#endif
};
...
{
    template<> inline FName GetKeyTypeName<FEvtScriptPayload>()
    {
        return "FEvtScriptPayload";
    }
}
#endif