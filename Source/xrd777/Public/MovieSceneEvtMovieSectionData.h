...
public:
	 ...
	 */
	FORCEINLINE TMovieSceneChannelData<FEvtMoviePayload> GetData()
	{
		return TMovieSceneChannelData<FEvtMoviePayload>(&Times, &KeyValues, &KeyHandles);
	}
	 ...
	 */
	FORCEINLINE TMovieSceneChannelData<const FEvtMoviePayload> GetData() const
	{
		return TMovieSceneChannelData<const FEvtMoviePayload>(&Times, &KeyValues);
	}
	...
	bool Evaluate(FFrameTime InTime, FEvtMoviePayload& OutValue) const { return true; }
...
public:
	...
	const TArray<FEvtMoviePayload>& GetValues() const { return KeyValues; }
public:
    FMovieSceneEvtMovieSectionData();
};

template<>
struct TMovieSceneChannelTraits<FMovieSceneEvtMovieSectionData> : TMovieSceneChannelTraitsBase<FMovieSceneEvtMovieSectionData>
{
#if WITH_EDITOR
    /** Byte channels can have external values (ie, they can get their values from external objects for UI purposes) */
    typedef TMovieSceneExternalValue<FEvtMoviePayload> ExtendedEditorDataType;
#endif
};
...
{
    template<> inline FName GetKeyTypeName<FEvtMoviePayload>()
    {
        return "FEvtMoviePayload";
    }
}
#endif